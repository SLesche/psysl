...
You can print tables using the wonderful `apa_table` command provided to you by `papaja` (see \@ref(tab:results-example-table)). Here it is best to set the caption using the `caption` argument provided by `apa_table`

```{r results-example-table}
apa_table(best_cars_table, caption = "The top five cars by Miles Per Gallon (MPG)")
```
